...
<div>
	...
	{#if $open}
		<ul
			...
			class="z-10 p-1 rounded-md bg-[--temp-ink-black] ring-4 ring-[--temp-paper]"
		>
			{#each options as sampleOption}
				<li
					use:melt={$option({ value: sampleOption, label: sampleOption.name })}
					class="font-medium trim-inter p-3 rounded-sm text-[--temp-highlight] data-[selected]:bg-[--temp-ink] data-[highlighted]:bg-[--temp-ink-dark] data-[highlighted]:ring-4 ring-[--temp-ink] cursor-pointer"
				>
					{sampleOption.name}
				</li>
			{/each}
